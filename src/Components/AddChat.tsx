...
export function AddChat({ showModal, setShowModal, setChats }: {
    ...
    const AddChatSelection = () => {
        return <div className="gap-2 flex flex-col md:flex-row w-full h-full">
            <div className="border-white bg-gray-800 grow relative group flex flex-col  cursor-pointer" onClick={(e) => {
    ...
    return (<>
        ...
        <div className={`absolute h-5/6 w-3/4 md:h-3/4 md:w-2/4 p-8 top-0 bottom-0 right-0 left-0 m-auto z-30 dark:bg-gray-900 dark:text-gray-200 ${showModal ? '' : 'hidden'}`} >
            <AddChatContent />
        </div>
    </>);
}
